export default function ThisType(): JSX.Element {
	return (
        ...
        <header><code className="header-code">{`ThisType<Type>`}</code></header>
        <article>
            <div>
...
obj.moveBy(5, 5);
            ...
            <section>
                <p>In the example above, the <code className="header-code">methods</code> object in the argument to
                    <code className="header-code">makeObject</code> has a contextual type that includes <code
                        className="header-code">{`ThisType<D & M>`}</code> and therefore the type of <span><a
                            className="text-linked" target="_blank"
                            href="https://www.typescriptlang.org/docs/handbook/functions.html#this"></a>this</span> in
                    methods within the <code className="header-code">methods</code> object is <code
                        className="header-code">{`{ x: number, y: number } & { moveBy(dx: number, dy: number): void }`}</code>.
                    Notice how the type of the methods property simultaneously is an inference target and a source for
                    the <code className="header-code">this</code> type in methods.</p>
                <p>The <code className="header-code">{`ThisType<T>`}</code> marker interface is simply an empty interface
                    declared in <code className="header-code">lib.d.ts</code>. Beyond being recognized in the contextual
